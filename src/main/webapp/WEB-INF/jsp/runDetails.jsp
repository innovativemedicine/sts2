...
<%@ include file="/WEB-INF/jsp/includes/success.jsp"%>
	...
	<c:param name="runId" value="${command.runId}"/></c:url>"
		onclick="return (confirm('Are you sure you want to delete this run? Read the warning carefully before you confirm!')) ">
			Delete it</a> <font color="red">Warning:</font> Deleting a run will also
		delete the results of that run from the database !
		<p>
		<p>
		<h2>Run Details</h2>
		</p>
  	...
  	<c:param name="runId" value="${command.runId}"/></c:url>">
			Genotype result of this run </a><br> <a
		href="<c:url value="/resultList.htm">
  	<c:param name="runId" value="${command.runId}"/>
  	<c:param name="linkageFormat" value="yes"/></c:url>">
			Genotype result of this run in linkage output format</a><br> <br>
	<br>
		<h3>Assay List</h3>
