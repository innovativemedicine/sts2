 ...
 * @author Gloria Deng
 * 
 *         This class is the middleclass between web object and DAO. It is also
 *         the target class spring framework provide transaction management
 */
public class SampleManager implements ConstantInterface {
	...
	public void removePatient(String intSampleId) {
		patientDAO.removePatient(intSampleId);
	}
	...
	public void removeSamplesInContainer(Integer sicId) {
		// Edit Container Content: Remove the sample from container by SIC.
		// Keeps sample record.
	...
	public void removeSamplesInContainerByContainer(Integer containerId) {
		// Empty Container: Remove the sample from container by container ID.
		// Keeps sample record
	...
	public void saveSample(Sample sample) throws Exception {
		...
		if (sample.getSampleId().intValue() == -1) {
			...
			if (!patientDAO.containsPatient(patient.getIntSampleId())) {
				...
				List existingSample = sampleDAO.getSampleByIntSampleIdSampleType(sample.getPatient().getIntSampleId(),
						sample.getSampleType());

				if (!existingSample.isEmpty()) {
					sample.setSampleDupNo(existingSample.size() + 1);
					// throw new
					// Exception("Error: Sample already exists. Cannot save sample.");
				}
			}
		}
	 ...
	 * @param sampleType
	 * @param samples
	 *            key id the well location in plate, value is sample return the
	...
	public List saveSamples(Container container, SampleType sampleType, Map samples, boolean setWell) throws Exception {
		...
		while (i.hasNext()) {
			...
			Sample sample1 = sampleDAO.getSampleByIntSampleIdUniKey(sample.getPatient().getIntSampleId(), sampleType,
					sample.getSampleDupNo());
			// check here in order to get a whole list of troubled samples once
			if (sample1 != null) {
				result += "duplicate sample:" + sample.getPatient().getIntSampleId() + sampleType.getSuffix()
						+ sample.getSampleDupNo() + "<br>";
			}
	...
	public void setSamplesInContainerDAO(SamplesInContainerDAO samplesInContainerDAO) {
