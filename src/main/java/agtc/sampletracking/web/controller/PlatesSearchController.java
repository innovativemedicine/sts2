...
public class PlatesSearchController extends BasicSearchController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request,
		...
		if (useFor.equals("useFor")) {
			SampleListHolder plateListHolder = (SampleListHolder) WebUtils
					.getOrCreateSessionAttribute(request.getSession(),
							"plateListHolder", SampleListHolder.class);
			plateListHolder.setOriginalList(searchResults);
			ModelAndView view = new ModelAndView(new RedirectView(
					"plateList4select.htm"));
			return view;
		}

		if (searchResults.size() > 1) {
			// multiple projects found
			return new ModelAndView("containerList", "containerList",
					searchResults);
		}
	...
	protected Map referenceData(HttpServletRequest request) throws Exception {
		...
		List containerList = containerManager.getAllPlates();
