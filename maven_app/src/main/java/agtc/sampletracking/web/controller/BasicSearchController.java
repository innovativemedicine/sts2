...
public abstract class BasicSearchController extends BasicController {
	...
	protected List handleSubmit(HttpServletRequest request, Object command, String entityName) {
		...
		List crtList = (List) WebUtils.getOrCreateSessionAttribute(request.getSession(), criteriaListName,
				ArrayList.class);

		List lgcList = (List) WebUtils.getOrCreateSessionAttribute(request.getSession(), logicalListName,
				ArrayList.class);
		String searchField = criteria.getSearchField();
		String operator = criteria.getOperator();
		if (!searchField.equals("select to search") && !operator.equals("select to search")) {
		...
		if (action.equals("SEARCH")) {
			...
			} else {// only left choise is clear, will clean all criteria in the
					// session
				crtList = new ArrayList();
				lgcList = new ArrayList();
			}
	...
	protected void referenceData(javax.servlet.http.HttpServletRequest request, Map models, String entityName)
