...
public class BarcodeController extends BasicController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command,
		...
		// Handle Submit is where all the actions are processed. Defines all the
		// processing in barcodeManager
	...
	protected String processBarcode(BarcodeCommand barcodeItem, String storeContainer) {
		...
		if (barcodeString.matches("S-" + SAMPLE_PREFIX + "\\d+-[a-zA-Z]{2,3}")) {
			...
			if (sample == null) {
				// response += addSampleFromBarcode(sampleId, sampleTypeSuffix,
				// sampleDupNo);
			...
			if (container == null) {
				...
				// To make this work properly, need to open a add Container form
				// and disable barcodes until form is filled.
	...
	protected String updatePlateFromBarcode(Container plate, String storeContainer) {
		...
		if (storeContainer.isEmpty()) {
			if (plateStatus.equalsIgnoreCase("registered")) {
				...
				Set cic = motherContainer.getChildContainers(); // cics =
																// containers in
																// container
				cic.remove(plate);
	...
	protected String updateSampleFromBarcode(Sample sample, String storeContainer) {
		...
		if (storeContainer.isEmpty()) {
			...
			try {
				container = sampleManager.storeSampleInContainer(container, sample, "");
			} catch (Exception e1) {
	...
	// protected String addSampleFromBarcode(String sampleId, String
	// sampleTypeSuffix, Integer sampleDupNo)
	// {
	// String response = "";
	//
	...
	protected void handleSubmit(HttpServletRequest request, Object command, Map model) {
		...
		List actionList = (List) WebUtils.getOrCreateSessionAttribute(request.getSession(), "actionedList",
				ArrayList.class);
		String storeContainer = (String) WebUtils.getOrCreateSessionAttribute(request.getSession(), "storeContainer",
				String.class);
	...
	protected Map referenceData(HttpServletRequest request) throws java.lang.Exception {
		...
		List actionedList = (List) WebUtils.getOrCreateSessionAttribute(request.getSession(), "actionedList",
				ArrayList.class);
